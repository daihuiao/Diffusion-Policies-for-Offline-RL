...
dataset2 = Dataset(np.random.multivariate_normal(mean2, cov2, 50000), device=device)
...
model2 = MLP_res(state_dim=2, action_dim=2, device=device)
actor2 = Diffusion(state_dim=1, action_dim=2, model=model2, max_action=1.0,
                   beta_schedule='vp', n_timesteps=n_timesteps, loss_type="l2").to(device)
actor2_optimizer = torch.optim.Adam(actor2.parameters(), lr=lr)

def train(index, actor, actor_optimizer, dataset, epoch, iterations=1000):
    for _ in range(iterations):
        if random.random() < 0.5:  # 0.9的概率用真实label
...
for i in trange(1000):
    ...
    train(2, actor2, actor2_optimizer, dataset2, epoch=i, iterations=100)
    for key, parameter in actor0.model.state_dict().items():
        global_parameters_actor[key] = parameter.clone()
    for key, parameter in actor1.model.state_dict().items():
        global_parameters_actor[key] += parameter.clone()
    for key, parameter in actor2.model.state_dict().items():
        global_parameters_actor[key] += parameter.clone()
