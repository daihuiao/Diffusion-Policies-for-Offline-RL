...
def dataset_info(dataset):
    ...
    for i in range(dataset["observations"].shape[0]):
        if not dataset["terminals"][i]:
            ...
            raise Exception
    sub_datasets = []
    intervel = dataset["observations"].shape[0] // args.num_agents
...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    if args.algo == 'ql':
        from agents.ql_diffusion_vae_bc import Diffusion_QL_vae_bc as Agent
        agents = []
        for i in range(args.num_agents):
...
if __name__ == "__main__":
    ...
    # Setup Logging
    file_name = f"bc_vae|agent-{args.num_agents}|{args.env_name}|{args.exp}|diffusion-{args.algo}|T-{args.T}|"
