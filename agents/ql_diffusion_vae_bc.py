...
class Actor(nn.Module):
    def __init__(self,state_dim, action_dim,latent_dim, max_action, device):
        ...
        self.model= VAE(state_dim=state_dim, action_dim=action_dim, latent_dim=latent_dim, max_action=max_action, device=device).to(device)
    def forward(self, state, action):
        return self.model(state, action)
    def decode(self, state, z=None):
        return self.model.decode(state, z)

class Diffusion_QL_vae_bc(object):
    def __init__(self,
                 state_dim,
                 action_dim,
                 max_action,
        ...
        self.actor =  Actor(state_dim=state_dim, action_dim=action_dim, latent_dim=action_dim*2, max_action=max_action, device=device).to(device)
    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None, id=0):
        ...
        for _ in trange(iterations):
            ...
            if self.max_q_backup:
                ...
                next_action = self.ema_model.decode(next_state)
            ...
            new_action = self.actor.decode(state)
