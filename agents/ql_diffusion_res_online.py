...
class Diffusion_QL_res_online(object):
    def __init__(self,
                 state_dim,
                 action_dim,
                 max_action,
    ...
    def train(self, replay_buffer, iterations, batch_size=100, log_writer=None, id=0):
        ...
        return metric
    def train_only_actor(self, replay_buffer, iterations, batch_size=100, log_writer=None, id=0):
    ...
    def save_model(self, dir, id=None):
        if id is not None:
            ...
            torch.save(self.diffusion.state_dict(), f'{dir}/diffusion_{id}.pth')
    ...
    def load_model(self, dir, id=1000):
        if id is not None:
            self.actor.load_state_dict(torch.load(f'{dir}/actor_{id}.pth'))
            self.diffusion.load_state_dict(torch.load(f'{dir}/diffusion_{id}.pth'))
            self.ema_model.load_state_dict(torch.load(f'{dir}/actor_{id}.pth'))
            self.critic.load_state_dict(torch.load(f'{dir}/critic_{id}.pth'))
            self.critic_target.load_state_dict(torch.load(f'{dir}/critic_{id}.pth'))
        else:
