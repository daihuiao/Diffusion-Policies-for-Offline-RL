...
def train(index, actor, actor_optimizer, dataset, epoch, iterations=1000):
    for _ in range(iterations):
        if random.random() < 0.8:  # 0.9的概率用真实label
...
def plot_all(epoch):
    ...
    plt.scatter(x, y)
    plt.axis('equal')
    plt.title(f"epoch:{epoch},n_timesteps:{n_timesteps},iterations:{iterations}")
    plt.show()
    haha = True
...
iterations = 100
for i in trange(1000*2):
    train(0, actor0, actor0_optimizer, dataset0, epoch=i, iterations=iterations)
    train(1, actor1, actor1_optimizer, dataset1, epoch=i, iterations=iterations)
    for key, parameter in actor0.model.state_dict().items():
        global_parameters_actor[key] = parameter.clone()
