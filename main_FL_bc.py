...
def train_agent(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    if args.algo == 'ql':
        from agents.ql_diffusion_bc import Diffusion_QL_bc as Agent
        agents = []
        for i in range(args.num_agents):
...
if __name__ == "__main__":
    ...
    # Setup Logging
    file_name = f"bc_diffusion|agent-{args.num_agents}|{args.env_name}|{args.exp}|diffusion-{args.algo}|T-{args.T}"
